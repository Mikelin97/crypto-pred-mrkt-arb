...
def backtest_market(crypto_data_filepath, crypto_data_filename, crypto_symbol, 
        ...
        backtest_start_time = adjust_precision(backtest_start_time, time_precision)
        backtest_end_time = adjust_precision(backtest_end_time, time_precision)
        warm_up_start_time = backtest_start_time - warm_up_duration
        # warm_up_duration = adjust_precision(warm_up_duration, time_precision) -> can't because it's a time difference
        
        # read in crypto data
        all_crypto_df = crypto_raw_to_pandas(f"{crypto_data_filepath}{crypto_data_filename}")
        crypto_df = all_crypto_df[all_crypto_df['symbol'] == crypto_symbol].sort_values(by="unix_timestamp").reset_index(drop=True)
        ...
        in_scope_crypto_df = crypto_df[(crypto_df['unix_timestamp'] >= warm_up_start_time) & (crypto_df['unix_timestamp'] <= backtest_end_time)]
        crypto_price_queue = deque(zip(in_scope_crypto_df['unix_timestamp'], in_scope_crypto_df['value']))
        market_update_queue = deque(yes_book_updates)
        trade_queue = deque(trades_yes_perspective)
        ...
        while trade_queue and trade_queue[0]['timestamp'] < warm_up_start_time:
            trade_queue.popleft()

        # warm up strategy
        while crypto_price_queue or market_update_queue or trade_queue:
            ...
            if trade_queue:
                next_trade_time = trade_queue[0]['timestamp']
                next_times.append(next_trade_time)
            
            next_timestamp = min(next_times)

            if next_timestamp >= backtest_start_time:
            ...
            market_updates = []
            while market_update_queue and market_update_queue[0]['timestamp'] == next_timestamp:
                market_update = market_update_queue.popleft()
                market_updates.append(market_update)
            
            trades = []
            while trade_queue and trade_queue[0]['timestamp'] == next_timestamp:
                trade = trade_queue.popleft()
                trades.append(trade)

            strategy.on_warmup(next_timestamp, latest_crypto_price, market_updates, trades)
            warm_up_data.append({
        ...
        while crypto_price_queue or market_update_queue:
            ...
            if next_timestamp > backtest_end_time:
            ...
            strategy.on_update(next_timestamp, latest_crypto_price, market_updates, trades)
