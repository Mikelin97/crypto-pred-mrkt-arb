...
class ExpiringMarketRevertStrategy(Strategy):
    """
        This market does the opposite of the expiring market stuff. Instead of picking up pennies
        in front of a steamroller, we play the lottery expecting that the markets are mispricing the
        reversion probability
    """
    
    
    def __init__(self, market_strike_set_time, market_resolve_time, time_precision, max_spread, effective_memory=300):
    ...
    def on_update(self, timestamp, crypto_price, book_updates, trades):
        ...
        if not np.isnan(self.resolve_price):
            return

        (bid_price, bid_size), (ask_price, ask_size) = self.order_book.top_market
        if self.position == 0 and self.order_book.spread <= self.max_spread and ask_price == 0.99:
            # take a position if not in one
            self.trades.append({"type": "sell", "timestamp": timestamp, "price": bid_price, "size": bid_size})
            self.position = -bid_size
            self.entry_price = bid_price
        elif self.position == 0 and self.order_book.spread <= self.max_spread and bid_price == 0.01:
            # take a position if not in one
            self.trades.append({"type": "buy", "timestamp": timestamp, "price": ask_price, "size": ask_size})
            self.position = ask_size
